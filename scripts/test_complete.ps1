...
try {
    ...
    Write-Host "   [OK] Master iniciado correctamente" -ForegroundColor Green
} catch {
...
Write-Host "   [OK] Workers registrados: $($response.workers.Count)" -ForegroundColor Green

# Prueba 1: Operacion legacy
Write-Host "`n4. Prueba: Operacion legacy (map_add)..." -ForegroundColor Cyan
...
if ($progress.status -eq "completed") {
    Write-Host "   [OK] Job completado: $($progress.completed_tasks)/$($progress.total_tasks)" -ForegroundColor Green
} else {
    Write-Host "   [ERROR] Job no completado: $($progress.status)" -ForegroundColor Red
}

# Prueba 2: flat_map
Write-Host "`n5. Prueba: Operacion flat_map..." -ForegroundColor Cyan
...
if ($progress.status -eq "completed") {
    Write-Host "   [OK] flat_map completado: $($progress.completed_tasks)/$($progress.total_tasks)" -ForegroundColor Green
} else {
    Write-Host "   [ERROR] flat_map no completado: $($progress.status)" -ForegroundColor Red
}

# Prueba 3: reduce_by_key
Write-Host "`n6. Prueba: Operacion reduce_by_key..." -ForegroundColor Cyan
...
if ($progress.status -eq "completed") {
    Write-Host "   [OK] reduce_by_key completado: $($progress.completed_tasks)/$($progress.total_tasks)" -ForegroundColor Green
} else {
    Write-Host "   [ERROR] reduce_by_key no completado: $($progress.status)" -ForegroundColor Red
}

# Prueba 4: Metricas
Write-Host "`n7. Prueba: Metricas..." -ForegroundColor Cyan
try {
    $metrics = Invoke-RestMethod -Uri "http://127.0.0.1:8080/api/v1/metrics" -Method GET
    Write-Host "   [OK] Metricas obtenidas:" -ForegroundColor Green
    ...
    Write-Host "   [ERROR] Error obteniendo metricas: $_" -ForegroundColor Red
}

# Prueba 5: Estado de workers
Write-Host "`n8. Prueba: Estado de workers..." -ForegroundColor Cyan
$response = Invoke-RestMethod -Uri "http://127.0.0.1:8080/api/v1/workers" -Method GET
Write-Host "   [OK] Workers activos: $($response.workers.Count)" -ForegroundColor Green
$allUp = ($response.workers | Where-Object {$_.status -eq "UP"}).Count -eq $response.workers.Count
if ($allUp) {
    Write-Host "   [OK] Todos los workers estan UP" -ForegroundColor Green
} else {
    Write-Host "   [WARN] Algunos workers no estan UP" -ForegroundColor Yellow
}

# Resumen
Write-Host "`n=== RESUMEN DE PRUEBAS ===" -ForegroundColor Green
Write-Host "[OK] Master funcionando" -ForegroundColor Green
Write-Host "[OK] Workers registrados y activos" -ForegroundColor Green
Write-Host "[OK] Operaciones legacy funcionando" -ForegroundColor Green
Write-Host "[OK] Operaciones nuevas (flat_map, reduce_by_key) funcionando" -ForegroundColor Green
