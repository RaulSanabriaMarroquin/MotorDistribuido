...
async fn submit_job(
    ...
    let mut task_ids = Vec::new();
    {
        ...
        // For simplicity, create one task per node
        // In a real implementation, we'd create tasks based on partitions and parallelism
        for (idx, node) in payload.dag.nodes.iter().enumerate() {
            let task_id = format!("{}-task-{:03}", job_id, idx);
            ...
            let task_info = TaskInfo {
                ...
                input_paths: Vec::new(), // Will be set based on edges
                output_path: format!("output/{}/{}", job_id, task_id),
                partition: 0,
                created_at: SystemTime::now(),
                started_at: None,
                completed_at: None,
